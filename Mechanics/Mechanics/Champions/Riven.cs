...
namespace Mechanics
{
    class Riven : Champion
    {
        ...
        public Riven()
        {
            ...
            MecanicaUtil.PrintMessage(ObjectManager.Player.ChampionName + " Plugin Loaded!", MecanicaUtil.HexColor.AliceBlue);
        }
        ...
        private void Game_OnGameUpdate(EventArgs args)
        {
            ...
            switch (Program.Champion.Orbwalker.ActiveMode)
            {
        ...
        private static void UseRSmart(Obj_AI_Base target)
        {
            ...
            else if (target is Obj_AI_Hero)
            {
                ...
                if (po.Hitchance != HitChance.High)
                    return;
                if (ObjectManager.Player.GetSpellDamage(hero, SpellSlot.R) > ((targ.Health + target.ScriptHealthBonus) - 5 * targ.Level) && R.IsReady())
        ...
        {
            if (args.PacketData[0] != 154 || Orbwalking.OrbwalkingMode.Combo != Program.Champion.Orbwalker.ActiveMode)
                return;
            ...
            if (cast.Slot == SpellSlot.E && R.IsReady())
                Utility.DelayAction.Add(Game.Ping + 100, () => UseRSmart(Program.Champion.Orbwalker.GetTarget()));
        }
        ...
        public static void CancelAnim()
        {
            ...
            if (W.IsReady())
                UseWSmart(Program.Champion.Orbwalker.GetTarget());
            else
            {
        ...
        private void CastR()
        {
            ...
            else
            {
                ...
                if (!target.IsValidTarget(R.Range) || R.GetPrediction(target).Hitchance < HitChance.High)
                    return;
                if (ObjectManager.Player.GetSpellDamage(hero, SpellSlot.R) >= target.Health || Environment.TickCount - RTick >= 13000 || !target.IsValidTarget(R.Range - 400))
