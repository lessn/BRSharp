...
namespace Mechanics
{
    ...
    internal class Draven : Champion
    {
        ...
        {
            if (E.IsReady() && Menu.Item("EGapCloser").GetValue<bool>() && gapcloser.Sender.IsValidTarget(E.Range))
            {
        ...
        {
            if (E.IsReady() && Menu.Item("EInterruptable").GetValue<bool>() && unit.IsValidTarget(E.Range))
            {
                E.Cast(unit);
            }
        }
        ...
        {
            var drawOrbwalk = Menu.Item("DrawOrbwalk").GetValue<Circle>();
            var drawReticles = Menu.Item("DrawReticles").GetValue<Circle>();
            ...
            {
                Utility.DrawCircle(Game.CursorPos, Menu.Item("CatchRadius").GetValue<Slider>().Value, Color.Red);
            }
            else
            {
                Utility.DrawCircle(
                    Game.CursorPos, Menu.Item("CatchRadius").GetValue<Slider>().Value, Color.CornflowerBlue);
            }
        ...
        public override void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if (ComboActive)
            {
                ...
                if (IsFleeing(target) && Menu.Item("UseEC").GetValue<bool>())
                {
                    E.Cast(target);
                }
                ...
                try
                {
                    if (Menu.Item("UseRC").GetValue<bool>() && R.GetHealthPrediction(target) <= 0)
                    {
                        R.Cast(target);
                    }
                }
            ...
            //Manual cast R
            if (Menu.Item("RManualCast").GetValue<KeyBind>().Active)
            {
                R.Cast(rtarget);
            }
            ...
            //Peel from melees
            if (Menu.Item("EPeel").GetValue<bool>()) //Taken from ziggs(by pq/esk0r)
            {
        ...
        public override void Orbwalking_AfterAttack(Obj_AI_Base unit, Obj_AI_Base target)
        {
            ...
            Console.WriteLine(Menu.Item("maxqamount").GetValue<Slider>().Value);
            var qOnHero = QBuffCount();
            if (unit.IsMe &&
                ((ComboActive && Menu.Item("UseQC").GetValue<bool>()) ||
                 (HarassActive && Menu.Item("UseQC").GetValue<bool>())) && qOnHero < 2 &&
                qOnHero + ExistingReticles.Count < Menu.Item("maxqamount").GetValue<Slider>().Value)
            {
                Q.Cast();
        ...
        public Vector3 GetOrbwalkPos()
        {
            ...
            var reticles =
                ExistingReticles.OrderBy(reticle => reticle.ExpireTime)
                    .FirstOrDefault(
                        reticle =>
                            reticle.ReticlePos.Distance(cursor) <= Menu.Item("CatchRadius").GetValue<Slider>().Value &&
